...
class Test(unittest.TestCase):
    ...
    def test_dog_fields(self):
        ...
        attrs = dog.__dict__
        self.assertTrue(
            "_name" in attrs, "Should have a private field named name"
        )
        self.assertFalse(
            "name" in attrs, "Should not have a public field named name"
        )
        self.assertEqual(
            "Chester", dog._name, error_message("Chester", dog._name)
        )
    ...
    def test_dogpack_constructor(self):
        ...
        self.assertTrue(
            "_dogs" in attrs, "Should have a private field named dogs"
        )

        self.assertEqual([], dogpack._dogs, error_message([], dogpack._dogs))
    ...
    def test_add_dog_(self):
        ...
        self.assertEqual(2, val, error_message(2, val, "Length of dogs field"))

        val = dogpack._dogs[0]._name
        self.assertEqual(
            "FirstDog", val, error_message("FirstDog", val, "First Dog's Name")
        )
        ...
        self.assertEqual(
            "SecondDog",
            val,
            error_message("SecondDog", val, "Second Dog's Name"),
        )
...
ThirdDog: Woof"""
        ...
        with open("CP4/lesson14/practice-dogpack/dog_pack.py") as f:
            file_contents = f.read()
            self.assertFalse(
                "._name" in file_contents,
                "DogPack should not refer to the private fields of Dog",
            )
